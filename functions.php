...
function twenty_posts_on_homepage( $query ) {
    if ( $query->is_home() && $query->is_main_query() ) {
        $outlets = array( 'tv', 'press', 'sport' );
        $posts_to_display = get_equal_num_posts_from_each_outlet( $outlets );
 ...
 */
function get_equal_num_posts_from_each_outlet( $outlets ) {
  $posts_to_display = array();
  foreach ( $outlets as $outlet ) {
    $outlet_query = new WP_Query( array(
      'category_name' => $outlet,
      'posts_per_page' => ($outlet === "tv")? 6 : 5,
      'fields' => 'ids'
    ));
